...
def find_document_by_role(
    archive: CourseArchive, *, role: str
) -> StoredFile | None:
    """Locate a document by a semantic role found in activity/file names."""
    marker = role.casefold().strip()
    for activity, stored_file in iter_files(archive):
        if marker in activity.name.casefold() or marker in stored_file.filename.casefold():
            return stored_file
    return None
