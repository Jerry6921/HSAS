...
from hsas.infrastructure.storage.json_store import read_json, write_json, write_model
from hsas.infrastructure.storage.course_publisher import CourseSnapshotTransaction
from hsas.infrastructure.moodle.progress import SyncProgress
from hsas.infrastructure.moodle.sync_store import record_sync_operation
from hsas.infrastructure.moodle.session_store import (
    load_moodle_session_status,
    record_moodle_session_status,
)
from hsas.infrastructure.moodle.cancellation import (
    MoodleSyncCancelled,
    raise_if_cancelled,
)
from hsas.domain.courses.change_detection import (
    CourseChangeSet,
...
def _validated_course_id(course_url: str, base_url: str) -> str:
    ...
    if target.scheme not in {"http", "https"} or target.netloc != base.netloc:
        raise ValueError("course-url must use the exact MOODLE_BASE_URL host")
    course_id = parse_qs(target.query).get("id", [None])[0]
    if not course_id or not course_id.isdigit():
        raise ValueError("course-url must contain a numeric ?id= course ID")
    return course_id
    ...
    *,
    ...
    async def run() -> None:
        async with persistent_context(active_settings, headless=False) as context:
            ...
            notify("Complete HKU login/SSO/MFA in the opened browser.")
            await asyncio.to_thread(
                wait_for_user,
                "After the Moodle dashboard is visible, press Enter here... ",
            )
...
class MoodleCourseGateway:
    """Concrete Moodle adapter for the application-owned CourseGateway port."""
