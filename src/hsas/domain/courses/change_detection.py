...
from .models import CourseArchive, StoredFile
from .base import StrictModel
from .archive_index import iter_activities, iter_files
...
def _compare_activities(
    ...
    for activity_id in sorted(old.keys() | new.keys()):
        ...
        for key in ACTIVITY_CONTENT_KEYS:
            old_value = _json_value(getattr(before, key))
            new_value = _json_value(getattr(after, key))
            if old_value != new_value:
                changes.append(
                    CourseChange(
                        kind="activity",
