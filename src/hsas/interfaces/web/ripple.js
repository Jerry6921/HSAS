/*
 * Canvas UI Ripple (Vanilla WebGL), vendored from https://canvasui.dev/.
 * Copyright Canvas UI contributors. Licensed under MIT + Commons Clause.
 * Type annotations were stripped for direct browser delivery by HIQS.
 */
const DEFAULTS = {
    amplitude: 0.5,
    speed: 0.65,
...
function createRipple(elements, options = {}) {
    ...
    function pruneRipples(delta) {
        ...
        for(let i = ripples.length - 1; i >= 0; i--){
            const rp = ripples[i];
            rp.age += delta;
            const gone = rp.age * speedPx > diag + width * 3 || Math.exp(-Math.max(config.decay, 0.05) * rp.age) * rp.amp < 0.012;
...
window.CanvasUIRipple = { createRipple, supportsHtmlInCanvas };
