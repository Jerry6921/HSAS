...
在课程概览的课件目录中，选择“复制最近 Lecture 提示词”，即可把该课程、当前资料目录和
检索范围交给 Agent。Agent 会从日历与课程来源中确认最近一次已经发生的 Lecture，再阅读
本地课件和文本副本，按相关性列出材料并标注具体路径、页码或 slide。

每张课件卡旁的“复制 AI 提示词”用于定位并总结单份材料。提示词会自动带入当前用户实际的
HIQS 资料目录、课程标识、课件相对路径和文本副本地址；这些地址由程序在运行时解析，不绑定
README 作者的电脑或用户名。将复制结果直接粘贴给能够访问本机项目的 Agent 即可。
