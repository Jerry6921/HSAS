...
export function ReconciliationIsland({ options }: { options: ModernReconciliationOptions }) {
  ...
  return <div className="hiqs-modern-reconciliation">
    ...
    <div className="hiqs-reconciliation-list">
      {options.courses.map((course, index) => <motion.article key={course.courseCode} className={course.state} initial={reduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .025 }}>
